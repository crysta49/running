...
<div id="board">
    <table class="list" summary="번호, 제목, 첨부파일, 작성일, 조회수로 구성">
        <caption>${confBean.bbsNm} 목록</caption>
        <colgroup>
        ...
        <c:forEach items="${pagerBean.list}" var="item" varStatus="status"> 
            <tr>
                ...
                <td>
                    <c:set var="openClass" value="${item.openYn == 'Y' ? 'open_list' : 'nopen_list'}" />
                    <span class="${openClass}">${item.openYn == 'Y' ? '공개' : '비공개'}</span>
                </td>
                ...
                <td><sm:mask-word word="${item.writerNm}"/></td>
                <td>${item.regDt}</td>
                <td>${item.readCnt}</td>
            </tr>
        </c:forEach>
      ...
      <!--버튼-->
      <div class="rbt"><a href="#" onclick="BBS.form(); return false;"><img src="<c:url value="/resources/front/FE2/images/board/bt_bwrite.gif"/>" class="cl" alt="글쓰기" /></a></div>
