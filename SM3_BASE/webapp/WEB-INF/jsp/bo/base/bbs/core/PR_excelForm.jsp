            ...
            <div>
                <table class="tbl-excel">
                    <tr>
                        ...
                        <td><input type="checkbox" name="chk" id="mgrNm" class="smf-checkbox" value="writerNm" /><label for="writerNm">작성자</label></td>
                    ...
                    <c:if test="${confBean.typeCd == BBS_CONF_UTIL.QNA }">
                        <tr>
                            <td><input type="checkbox" name="chk" id="replyNm" class="smf-checkbox" value="replyNm"/><label for="replyNm">답변자</label></td>
                            <td><input type="checkbox" name="chk" id="statusNm" class="smf-checkbox" value="statusNm"/><label for="statusNm">처리상태</label></td>
                            <td>&nbsp;</td>
                        </tr>
                    </c:if>
                    ...
                    <%-- 
                    * 사용자 테이블 사용 시 샘플 코드
                    * 해당 테이블의 사용자 칼럼을 기입하도록 함
                    <tr>
                        <td><input type="checkbox" name="chk" id="VIEW_OPTION" class="smf-checkbox" value="VIEW_OPTION" /><label for="VIEW_OPTION">VIEW_OPTION</label></td>
                        <td><input type="checkbox" name="chk" id="SRC" class="smf-checkbox" value="SRC" /><label for="SRC">SRC</label></td>
                        <td>&nbsp;</td>
                    </tr>
                    --%>
                </table>
...
<script type="text/javascript">
    ...
    var js_checkDefault = function() {
        ...
        $("#title, #mgrNm, #readCnt, #regDt").prop("checked", true);
    };
    ...
    var js_excelConvertAction = function(btn) {
        ...
        if ( columnsCd.length < 1 ) {
            ...
            if ( confirm("엑셀파일로 내려받으시겠습니까?") ) {
                ...
                // 부모 페이지 검색조건 획득
                var condition = parent.BBS.getSearchCondition();
