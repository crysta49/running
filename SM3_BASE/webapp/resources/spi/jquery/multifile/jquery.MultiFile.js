...
if (window.jQuery)(function ($) {
			...
			.each(function () {
				...
				MultiFile.addSlave = function (slave, slave_count) {
					...
					slave.change(function (a, b, c) {
                        ...
                        // console.log(s + "=>" + sl(s));
						console.log(
						        "* MultiFile.maxsize : " + MultiFile.maxsize + 
						        "\n * newfs.size : " + newfs.size +
						...
						if (MultiFile.maxsize > 0 && total_size > MultiFile.maxsize) {
						    ...
						    //ERROR[ERROR.length] = MultiFile.STRING.toomuch.replace('$size', total_size + ' > ' + MultiFile.maxsize);
										...
										if(f!=null && f!=undefined){
											...
											$.each(files, function (i, file) {
												if(file.name!=undefined) {
													...
													/* Added by finkle */
													MultiFile.total_size += file.size;
												}
											});
										};
									};
	...
	*/
	$.fn.MultiFile.options = { //$.extend($.fn.MultiFile, { options: {
		accept: __MF_ACCEPT_EXTENSION, // accepted file extensions
		max: -1, // maximum number of selectable files
		maxfile: -1, // maximum size of a single file (KB)
		maxsize: __MF_MAX_SIZE, // maximum size of entire payload (KB)
		...
		STRING: {
			remove: '<span class="t3-delete" title="첨부 취소">첨부 취소</span>',
			denied: '이 확장자 [$ext]는 첨부가 제한되었습니다.\n다시 선택해주세요.',
			file: '$file',
			selected: '선택한 파일: $file',
			duplicate: '이미 선택한 파일입니다.:\n[ $file ]',
			toomany: '최대 $max 개의 파일만 첨부할 수 있습니다.',
            toomuch: '업로드 가능한 최대 크기를 초과했습니다.\n($size)',
			toobig: '$file : 업로드 최대 크기 초과 (max $size)'
		},
