    ...
    <form:form commandName="dataBean" id="updateForm" name="updateForm" autocomplete="off" onsubmit="false">
        ...
        <table class="tbl-view tbl-form">
            ...
            <tr>
                <th>부서 코드</th>
                <td class="plain">
                    <span id="nodeId">${dataBean.deptCd}</span>
                </td>
                <th>정렬 순서
                    ...
                    <div class="sr-hidden">
                        ...
                        <div class="smtip-text">
                            <div>좌측 부서목록에서 드래그 앤 드랍을 통해 정렬순서를 변경할 수 있습니다.</div>
                        </div>
                    </div>
                </th>
            ...
            <tr>
                <th>부서 명<em>&nbsp;</em></th>
                <td colspan="3">
                    <form:input path="text" size="40" cssClass="smf-text" onkeypress="if(event.keyCode==13) {js_enterKeyCheck('update'); return false;}"/>
                </td>
            </tr>
            <c:if test="${CONSTANT.USE_BRANCH}">
                <tr>
                    ...
                    <td colspan="3" class="plain">
                        <div class="deptTreeDiv" id="branchNm"></div>
                    </td>
                </tr>
            </c:if>
            <tr>
...
<form:form id="insertForm" name="insertForm" onsubmit="false">
    ...
    <table class="tbl-header">
        <tr>
            <th><span class="title-big">부서 추가</span></th>
            <td class="tr"><span class="required">표시는 필수입력 항목입니다.</span></td>
        </tr>
    </table>
            ...
            <td>
                <div style="margin: 4px 0;">
                    <div><input type="radio" name="appendOption" id="isSame" value="isSame" class="smf-radio"
                        <c:if test="${!exist}">disabled="disabled"</c:if> /><label for="isSame">선택한 부서 동일 레벨에 추가</label></div>
        
                    <div><input type="radio" name="appendOption" id="isChild" value="isChild" class="smf-radio"
                        <c:if test="${!exist}">disabled="disabled"</c:if> /><label for="isChild">선택한 부서 하위 레벨에 추가</label></div>
        
                    <div><input type="radio" name="appendOption" id="isRoot" value="isRoot" class="smf-radio"
                        checked="checked" /><label for="isRoot">루트 부서에 추가</label></div>
                </div>
            </td>
        </tr>
    </table>
    ...
    Ext.onReady(function() {
        ...
        <c:if test="${CONSTANT.USE_BRANCH}">
            $(".deptTreeDiv").SM_TREECOMBO({
                dataListUrl: "JR_listAll.do",
                value: "${empty dataBean.branchCd ? smp.branchCd : dataBean.branchCd}",
                select: function(record) {
                    $("#branchCd").val(record.data.id);
                }
            });
        </c:if>
